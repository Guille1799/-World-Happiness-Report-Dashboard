...
6. **Secrets** (optional): private data paths as in [`.env.example`](.env.example). To **hide** the yellow “Demo mode” strip while still using `data/demo_whr.csv`, add `WHR_HIDE_DEMO_BANNER = "true"` in **App settings → Secrets** (TOML format).
