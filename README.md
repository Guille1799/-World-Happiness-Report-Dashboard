...
6. **Secrets** (optional): private data paths as in [`.env.example`](.env.example). To **hide** the yellow “Demo mode” strip while still using `data/demo_whr.csv`, add **one** of these at the **top level** of **App settings → Secrets** (TOML, no section header): `WHR_HIDE_DEMO_BANNER = true` or `WHR_HIDE_DEMO_BANNER = "1"`. Alias: `HIDE_DEMO_BANNER`. Save, then **Reboot** the app. If it still shows, check for typos or a nested `[section]` that moved the key.
